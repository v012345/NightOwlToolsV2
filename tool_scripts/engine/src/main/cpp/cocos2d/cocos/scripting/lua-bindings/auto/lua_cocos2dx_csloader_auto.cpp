...
int lua_cocos2dx_csloader_CSLoader_getWidgetName(lua_State* tolua_S)
{
    int argc = 0;
    cocos2d::CSLoader* cobj = nullptr;
    bool ok  = true;
    ...
    if (!cobj) 
    {
        tolua_error(tolua_S,"invalid 'cobj' in function 'lua_cocos2dx_csloader_CSLoader_getWidgetName'", nullptr);
        return 0;
    }
#endif
    ...
    if (argc == 1) 
    {
        ...
        ok &= luaval_to_object<cocos2d::ui::Widget>(tolua_S, 2, "ccui.Widget",&arg0, "cc.CSLoader:getWidgetName");
        if(!ok)
        {
            tolua_error(tolua_S,"invalid arguments in function 'lua_cocos2dx_csloader_CSLoader_getWidgetName'", nullptr);
            return 0;
        }
        std::string ret = cobj->getWidgetName(arg0);
        tolua_pushcppstring(tolua_S,ret);
        return 1;
    }
    luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d \n", "cc.CSLoader:getWidgetName",argc, 1);
    return 0;

#if COCOS2D_DEBUG >= 1
    tolua_lerror:
    tolua_error(tolua_S,"#ferror in function 'lua_cocos2dx_csloader_CSLoader_getWidgetName'.",&tolua_err);
#endif
...
int lua_register_cocos2dx_csloader_CSLoader(lua_State* tolua_S)
{
    ...
    tolua_beginmodule(tolua_S,"CSLoader");
        ...
        tolua_function(tolua_S,"getWidgetName",lua_cocos2dx_csloader_CSLoader_getWidgetName);
