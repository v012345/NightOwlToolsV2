...
uint32 XStrUtil::split_line(char * strSrc, int len, vector<string> &vItems)
{
	vItems.clear();
	int i = 1;
	int start = 0, end;
	while(i < len && strSrc[i] != 0)
	{
		end = i;
		if(strSrc[i] == '\n' && strSrc[i - 1] == '\r')
		{
			if(end >= start - 2)
			{
				...
				memcpy(tmp, strSrc + start, end - start - 1);
				tmp[end - start - 1] = 0;
				string s = tmp;
				vItems.push_back(s);
				delete tmp;
...
{
	char buf[128];
	ASSERT(!cpszFormat || strlen(cpszFormat) < 100);
	if (cpszFormat)
...
{
	char buf[256];
	ASSERT(!cpszFormat || strlen(cpszFormat) < 240);
	if (cpszFormat)
