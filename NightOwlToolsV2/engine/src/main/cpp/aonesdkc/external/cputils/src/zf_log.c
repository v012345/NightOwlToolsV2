...
#if ZF_LOG_USE_NSLOG
    extern void zflog_CallNSLog(const char* msg);
...
static char* __memccpy(void *dest, const void *src, int c, size_t n)
{
	...
	{
		const ptrdiff_t cp_count = (char*)pc - (char*)src + 1;
		memcpy(dest, src, cp_count);
		return (char*)dest + cp_count;
	}
}
...
static INLINE char *put_string(const char *s, char *p, char *const e)
{
	...
	char *const c = __memccpy(p, s, '\0', n);
	return 0 != c ? c - 1 : e;
}
