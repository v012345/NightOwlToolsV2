...
/// context.
void XEMD5::update(const byte* input, uint32 inputLen)
{
...
void XEMD5::final(byte(&digest)[16])
{
	byte bits[8];
	uint32 index, padLen;

	// Save number of bits.
	encode(bits, m_count, 2);
...
void XEMD5::transform(const byte* block)
{
...
/// a multiple of 4.
void XEMD5::encode(byte* output, const uint32* input, uint32 inputLen)
{
...
/// a multiple of 4.
void XEMD5::decode(uint32* output, const byte* input, uint32 inputLen)
{
...
void md5(const void* src, uint32 size, byte(&digest)[16])
{
	...
	XEMD5 md5_;
	md5_.update((const byte*)src, size);
	md5_.final(digest);
	return;
}
