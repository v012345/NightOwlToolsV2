...
static void doswap(int swap, void *p, int n)
{
 if (swap)
 {
  char *a=p;
...
static int l_unpack(lua_State *L) 		/** unpack(s,f,[init]) */
{
 ...
 while (*f)
 {
  ...
  case 'I':				
   ...
   UNPACKNUMBER(OP_LONG, long long)
 ...
 while (*f)
 {
  ...
  while (N--) switch (c)
  {
   ...
   case OP_BOOLEAN:
   {
       ...
       unsigned char v8 = v;
       doswap(swap,&v8,sizeof(v8));
       luaL_addlstring(&b,(void*)&v8,sizeof(v8));
       break;
   }
          
   PACKSTRING(OP_BSTRING, unsigned char)
   PACKSTRING(OP_WSTRING, unsigned short)
   PACKSTRING(OP_SSTRING, int)
