...
#define LOG_TAG "CrashReporter"
	...
	#include <jni.h>
	#include "platform/android/cputils_android.h"
	...
	//for multithread problem, cache it
	static jclass cacheReportClass;
...
void CrashReport::setCrashReporterType(int type) {
    ...
    if (clazz) {
        ...
        NSLog(@"Warning: Fail to get class by NSClassFromString(%@)", BUGLY_AGENT_CLASS);
    }
...
void CrashReport::initCrashReport(const char* appId, bool isDebug, CrashReport::CRLogLevel level) {
	if (!initialized) {
			...
			initialized = true;
			JavaVM* jvm = cputils_get_jvm();
			if (jvm == NULL) {
				LOGE("CrashReport JavaVM is null.");
				return;
			}
			JNIEnv* env = NULL;
			jvm->GetEnv((void**)&env, JNI_VERSION_1_6);
			if (env == NULL) {
				LOGE("CrashReport JNIEnv is null.");
				return;
			}
			...
			//get PluginWrapper
			LOGI("try get org.cocos2dx.plugin.PluinWrapper");
			jclass activityClass = env->FindClass("indi/cputils/Helper");
			if (activityClass == NULL) {
				LOGE("CrashReport PluginWrapper is Null");
				return;
			}
			...
			if (activity == NULL) {
				LOGE("CrashReport activity is Null");
				return;
			}
			//call channel set package name
        if (crashReporterType != 0) {
            
            LOGI("CrashReport set channel: %d", crashReporterType);
			...
			//call init
			LOGI("CrashReport init by bugly.jar");
			...
			jclass tmpReportClass = env->FindClass(CRASHREPORT_CLASS);
			cacheReportClass = (jclass) env->NewGlobalRef(tmpReportClass);
...
void CrashReport::reportException(int category, const char* type, const char* msg, const char* traceback) {
	...
	jmethodID reportMethod = env->GetStaticMethodID(cacheReportClass, METHOD_POST_EXCEPTION, METHOD_POST_EXCEPTION_PARAMETER);
	jstring typeStr = env->NewStringUTF(type);
	jstring msgStr = env->NewStringUTF(msg);
	jstring traceStr = env->NewStringUTF(traceback);
	env->CallStaticVoidMethod(cacheReportClass, reportMethod, category, typeStr, msgStr, traceStr, false);
...
void CrashReport::log(CrashReport::CRLogLevel level, const char * tag, const char * fmts, ...) {
	...
	LOGI("[cocos2d-x] set log: %s - %s", tag, msg);
	jmethodID method = env->GetStaticMethodID(cacheReportClass, METHOD_SET_LOG, METHOD_SET_LOG_PARAMETER);
	env->CallStaticVoidMethod(cacheReportClass, method, (int)level, env->NewStringUTF(tag), env->NewStringUTF(msg));
	if(attachThread){
		jvm->DetachCurrentThread();
	}
	return;
