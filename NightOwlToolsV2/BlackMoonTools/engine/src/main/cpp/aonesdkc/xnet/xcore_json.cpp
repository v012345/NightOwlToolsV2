...
#define __JSON_SERIALIZABLE__
#ifdef __JSON_SERIALIZABLE__
...
{
	if (beginDoc == NULL || endDoc == NULL || beginDoc > endDoc) return false;
...
static string int64_to_str(int64 val)
{
	char buf[128];
	sprintf(buf, "%lld", val);
	return buf;
...
static string uint64_to_str(uint64 val)
{
	char buf[128];
	sprintf(buf, "%llu", val);
	return buf;
...
static string real_to_str(double val)
{
	char buf[128];
	sprintf(buf, "%#.16g", val);
	return buf;
...
void XJsonWriter::write_value(XJsonValue& value)
{
	...
	case XJsonValue::intValue:
		m_doc += int64_to_str(value.as_int());
		break;
	case XJsonValue::uintValue:
		m_doc += uint64_to_str(value.as_uint());
		break;
	case XJsonValue::realValue:
		m_doc += real_to_str(value.as_real());
		break;
